# Example 087: Least-Squares Line y = mx + c
...
Use least squares when fitting a line or linear model that minimizes total squared prediction error.

## Simple real-world scenario

Given advertising spend and observed sales, fit y=mx+c so the line summarizes the overall linear trend and can estimate sales at a new spend level.
...
Large residuals receive extra influence because errors are squared, so outliers can strongly affect the fit.
